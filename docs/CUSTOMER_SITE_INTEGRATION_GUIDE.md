...
1. 가능한 경우 `index.html` 단독 삽입보다 `공통 템플릿(레이아웃)` 또는 `태그 매니저(GTM)` 배포가 더 안전하다.  
2. `index.html` 삽입은 정적 단일 엔트리 사이트에서는 유효하지만, CMS/SSR/멀티 템플릿 환경에서는 누락 위험이 크다.  
3. 배포 후 GhostLink에서 7일 이벤트 감지(`script requests`, `bridge events`)로 설치 성공 여부를 검증해야 한다.
...
1. Day 0: 템플릿/GTM로 스크립트 삽입  
2. Day 1: 설치 감지 및 첫 스캔 확인  
3. Day 2~7: Proof Center에서 ACR/Citation/AI Assist 측정  
4. Week 2+: Auto-Optimize 승인 루프로 개선-검증 자동화
