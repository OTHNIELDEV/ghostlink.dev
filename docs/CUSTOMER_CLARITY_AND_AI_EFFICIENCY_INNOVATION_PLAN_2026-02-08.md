...
현재 고객이 느끼는 핵심 문제는 아래 두 가지다.

1. 어떻게 써야 하는지 명확하지 않다.  
2. 써서 실제로 얼마나 좋아졌는지 증명되지 않는다.

이 문제는 기능 부족보다 `가치 전달 구조 부족`에서 발생한다.

## 2. 현재 상태 진단 (코드 기준)

현재 구현에는 강력한 기능이 이미 존재한다. 그러나 고객 체감까지 연결이 약하다.
...
GhostLink를 “AI SEO 생성 도구”에서 “AI 검색 성과 증명 엔진”으로 재정의한다.

90일 목표:

1. 신규 고객의 첫 가치 인지 시간(Time-to-First-Proof)을 3일 → 30분 이내로 단축  
2. “효율이 좋은지 모르겠다” 불만 비율 50% 이상 감소  
3. Free→Paid 전환에 직접 연결되는 증명 지표(ACR/Citation/AI Assist) 대시보드 도입

## 4. 혁신 제안 아키텍처 (4개 트랙)

## 트랙 A. Guided Activation OS (사용법 명확화)

핵심 아이디어: 고객이 생각하지 않아도 “다음 행동”이 보이는 운영체제형 온보딩.

구성:

1. Dashboard 상단 `Start Here` 패널 도입  
항목: `사이트 등록 → 스캔 완료 → 브릿지 설치 확인 → 질문세트 선택 → 첫 증명 실행`

2. Progress 상태를 퍼센트가 아닌 행동 단위로 표시  
예: `2/5 완료 (다음: 브릿지 설치 확인)`

3. 조직별 Onboarding 상태 저장  
...
1. Activation Rate: 가입 후 24시간 내 Step 3 완료 비율  
2. First Scan Completion Rate  
3. Time-to-First-Proof

## 트랙 B. Proof of Efficiency Engine (효율 증명)

핵심 아이디어: “좋아 보이는 점수”가 아니라 “실제 질의 결과와 전환 기여”를 증명한다.

고객이 보는 화면:

1. Proof Center (신규 탭/페이지)  
카드: `Answer Capture Rate`, `Citation Rate`, `AI Assist Rate`, `전주 대비 변화`

2. Before/After 증거 카드  
...
1. Action Pack 생성 시 “예상 KPI 영향” 선표시  
...
1. Action Approval Rate  
2. Action-to-Lift Conversion (`승인된 액션 중 실제 지표 개선 비율`)  
3. 반복 사용률 (`주당 2회 이상 실험 실행 조직 비율`)

## 트랙 D. ROI Narrative Layer (구매 설득 강화)

핵심 아이디어: 기술 지표를 매출/리드 지표로 번역하여 의사결정자 설득력을 높인다.
...
1. 상단 고정 `Start Here` + `Proof Now` 듀얼 CTA  
2. Visibility Score 단일 수치에서 `구성요소(콘텐츠/크롤러/답변점유)`로 분해  
...
3. 기술 문구를 행동 문구로 전환  
예: `Generate Actions` → `다음 72시간 성과 개선 실행`
...
고객 신뢰를 위해 “추정”과 “실측”을 분리해 표기해야 한다.

원칙:

1. 생성형 문구 기반 개선치(`+35%`)는 `Predicted` 라벨을 부착  
2. Answer Capture/Attribution 기반 값만 `Measured` 라벨 사용  
...
1. 온보딩 상태 모델 + API 구현  
2. Dashboard Start Here 레일 구현  
3. `traffic_visibility_score` UI 노출

Week 3-4 (증명 MVP):

1. Proof Center 페이지 생성  
2. Answer Capture/Attribution 집계 API 통합  
3. Before/After 카드 MVP
...
1. A군: 기존 대시보드  
2. B군: Start Here + Proof CTA

성공 기준:

1. 7일 내 첫 Proof 실행률 +25%  
2. 14일 내 재방문율 +15%
...
GhostLink의 핵심 과제는 기능 추가가 아니라 `가치 증명 UX의 제품화`다.  
이미 구현된 엔진(Answer Capture, Attribution, Auto-Optimize)을 고객 여정의 앞단에 재배치하면, 단기간에 체감 품질과 전환을 동시에 개선할 수 있다.
